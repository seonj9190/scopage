...
최초 설정(위 과정)이 끝난 뒤, 코드가 바뀔 때마다 서버에서 아래 한 줄만 실행하면
`git pull` → `npm ci` → `npm run build` → pm2 재시작까지 자동으로 처리됩니다.

```sh
./deploy/deploy.sh
```

커밋되지 않은 로컬 변경사항이 있으면(예: 서버에서 직접 파일을 수정한 경우)
안전을 위해 스크립트가 바로 중단되고 어떤 파일이 문제인지 보여줍니다. 먼저
`git status`로 확인 후 필요한 조치(커밋/되돌리기)를 하고 다시 실행하세요.
